...
# Per-provider assistant-turn label — same emoji as PROV_LABEL's folder badge, but phrased as a
# message-label ("Codex", not "🌀 Codex" folder badge text repurposed awkwardly) rather than the
# product name used for the folder chip. Falls back to ROLE_LABEL["assistant"] ("✦ Claude") for
# an unknown/unspecified provider so old call sites that don't pass `prov` keep working unchanged.
ASSISTANT_LABEL = {"codex": "🌀 Codex", "gemini": "✨ Gemini", "claude": "✦ Claude", "agy": "✨ Antigravity"}
ROLE_DESC = {
    "you": "Messages you actually typed or pasted — a verified ruleset marks only these as 'You'.",
    "assistant": "The assistant's replies.",
    "tool-result": "Output of a tool the agent ran (Bash command, Edit/Write, Read, …). Not written by a human.",
    ...
    "subagent": "Conversation of a sub-agent the agent spawned.",
    "orchestrator": "The task brief given to a sub-agent (generated by the agent, not a human).",
...
def legend_html(open_=False):
    rows = [("🧑 You", ROLE_DESC["you"]),
            ...
            ("✦ Agent", ROLE_DESC["assistant"]),   # neutral: this legend covers Claude/Codex/Gemini/Antigravity alike
            ("💭 Thinking", "The agent's reasoning — usually collapsed / private."),
            ("🔧 Tool call", "The agent calling a tool (run Bash, Edit/Write/Read a file, …)."),
...
SCOPES = {"all": "All", "human": "🧑 Only me", "claude": "✦ Only the agent",
...
class H(BaseHTTPRequestHandler):
    ...
    def index(self, proj_filter="", sort="date", dir_="", root=None, off=0):
        ...
        if proj_filter:
            ...
            # this workspace's sessions can span multiple providers (ckey merges providers into
            # one "All" workspace row above) — only name a single provider when every session here
            # actually is that provider; otherwise stay neutral rather than mislabel the mix
            item_provs = {it.get("provider", "claude") for it in items}
            asst_label = ASSISTANT_LABEL.get(next(iter(item_provs)), ROLE_LABEL["assistant"]) if len(item_provs) == 1 else "✦ Agent"
            ...
            statsblock = (
                ...
                + f'<div class=meta>{tr(asst_label)} {st["asst"]} · ⚙ {tr("tool results")} {st["tool"]}</div>'
        ...
        head = (f'<p class=meta>{len(items)} {tr("sessions")} · <b>🧑 {tr("You")}</b> {tr("marks — by a verified ruleset —")} '
                ...
                # this page can list sessions from every provider at once, so the legend blurb
